...
print('======================================')
print('=========钢梁水平埋件计算书word生成器=========')
print('=========内测版本20200525 by 徐明=======')
print('======================================')
print('提示：')
print('内爬钢梁门洞水平埋件计算修改版')
print('沿钢梁轴向的剪力对埋件不产生附加弯矩')
print('垂直于钢梁轴向的剪力会对埋件产生附加弯矩，弯矩作用中心高度同钢梁高度')
...
while True:
    try:
        xrow = int(input("水平锚筋排数[墙体厚度]: "))
        break
    except ValueError:
        print("输入错误，请输入正确数据[整数]")
while True:
    try:
        zrow = int(input("竖向锚筋排数[门洞宽度]: "))
        break
    except ValueError:
        print("输入错误，请输入正确数据[整数]")
while True:
...
while True:
    try:
        vx = int(input("x向剪力(N)[沿钢梁轴向,填正值,没有则填0]: "))
        break
    except ValueError:
        print("输入错误，请输入正确数据[整数]")
...
while True:
    try:
        vz = int(input("z向剪力(N)[垂直于钢梁方向,填正值,没有则填0]: "))
        break
    except ValueError:
        print("输入错误，请输入正确数据[整数]")
...
while True:
    try:
        vl = int(input("z向剪力作用点距离锚板平面的距离(mm): "))
        break
    except ValueError:
        print("输入错误，请输入正确数据[整数]")
...
mz = vx * 0  # z向弯矩设计值，由x向剪力产生，单位N.mm###此参数为0，沿钢梁轴向的剪力，不对水平埋件产生附加弯矩
...
if ny >= 0:  # 受拉工况校核
    ...
    as1 = vx / (ar * av * fy) + vz / (ar * av * fy) + ny / (0.8 * ab * fy) + mx / (1.3 * ar * ab * fy * zz)
    as2 = ny / (0.8 * ab * fy) + mx / (0.4 * ar * ab * fy * zz)
