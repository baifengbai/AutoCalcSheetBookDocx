...
print('[不计算杆件自重产生的弯矩]')
print('20210128修改公式图片生成代码 by 徐明')
print('')
...
while True:
    try:
        type1 = int(input("截面类型[1为圆管，2为方管]: "))
        if not (type1 == 1 or type1 == 2):
            print("输入错误，请输入正确的截面类型[1为圆管，2为方管]")
            continue
        break
    except ValueError:
        print("输入错误，请输入正确的截面类型[1为圆管，2为方管]")
...
while True:
    try:
        d1 = int(input("截面直径或边长(mm): "))
        break
    except ValueError:
        print("输入错误，请输入正确的数据[整数]")

while True:
    try:
        t1 = float(input("壁厚(mm): "))
        break
    except ValueError:
        print("输入错误，请输入正确的数据")
...
while True:
    try:
        steel = int(input("钢材牌号[235或345]: "))
        if steel == 235:
            xik1 = 1  # 钢号修正系数,Q235=1, Q345=0.825
        elif steel == 345:
            xik1 = 0.825
        else:
            print("输入错误，请输入正确的钢材牌号[235或345]")
            xik1 = 0.825
            continue
        break
    except ValueError:
...
while True:
    try:
        lam = int(input("压杆许用长细比[一般取150]: "))
        break
    except ValueError:
        print("输入错误，请输入正确的数据[整数]")

if steel == 235:  # 钢材的抗拉，抗压，抗弯强度设计值
    if t1 <= 16:
        fp = 215
    elif 16 < t1 <= 40:
        fp = 205
...
if type1 == 1:  # 截面属性，1为圆管，2为方管
    ...
    calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/gxj.jpg', width=Inches(width))
    mathtemp = r'i_x = ' + str(round(hzbj, 1)) + r'(mm)'
    width = add_image(mathtemp, 'hzbj')
    calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/hzbj.jpg', width=Inches(width))
    mathtemp = r'A = ' + str(round(mj, 1)) + r'(mm^2)'
    width = add_image(mathtemp, 'mj')
    calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/mj.jpg', width=Inches(width))
    ...
    if lambda1 <= 150:
        mathtemp = r'\lambda = \frac{L}{i_x} = \frac{' + f'{l1}'+'}{'+f'{round(hzbj, 1)}' + '}=' + str(
            ceil(lambda1)) + r'\leq [\lambda] =' + f'{lam}'
        width = add_image(mathtemp, 'lambda1')
        calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/lambda1.jpg',
                                                                                width=Inches(width))
    ...
    calc_book.add_paragraph('查规范得稳定系数：', style='Normal')
    mathtemp = r'\psi = ' + str(psi[ceil(lambda1 / xik1)])
    width = add_image(mathtemp, 'psi')
    calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/psi.jpg',
                                                                            width=Inches(width))
    ...
    mathtemp = r'\sigma = \frac{N}{A} = \frac{' + f'{n1}'+'}{'+f'{round(mj, 1)}' + '}=' + str(round(n1 / mj, 1)) + r'(MPa)'
    width = add_image(mathtemp, 'sigma')
    calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/sigma.jpg',
                                                                            width=Inches(width))
    calc_book.add_paragraph('稳定性：', style='Normal')
    sigma2 = n1 / (psi[ceil(lambda1 / xik1)] * mj)
    if sigma2 < fp:
        mathtemp = r'\frac{N}{\psi A} = \frac{' + f'{n1}'+'}{'+rf'{psi[ceil(lambda1 / xik1)]} \times {round(mj, 1)}' + r'}=' + str(
            round(sigma2, 1)) + r'(MPa) < f =' + f'{fp}(MPa)'
        width = add_image(mathtemp, 'sigma2')
        calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/sigma2.jpg',
                                                                                width=Inches(width))
        calc_book.add_paragraph('满足规范。', style='Quote')
    else:
    ...
    if lambda1 <= 150:
        mathtemp = r'\lambda = \frac{L}{i_x} = \frac{' + f'{l1} '+'}{'+f'{round(hzbj, 1)}' + r'}=' + str(
            ceil(lambda1)) + r'\leq [\lambda] =' + f'{lam}'
        width = add_image(mathtemp, 'lambda1')
        calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/lambda1.jpg',
                                                                                width=Inches(width))
    ...
    #                                                                         width=Inches(width))
    calc_book.add_paragraph('轴向应力：', style='Normal')
    mathtemp = r'\sigma = \frac{N}{A} = \frac{' + rf'{n1}'+'}{'+f'{round(mj, 1)}' + r'}=' + str(round(n1 / mj, 1)) + r'(MPa)'
    width = add_image(mathtemp, 'sigma')
    calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/sigma.jpg',
                                                                            width=Inches(width))
    calc_book.add_paragraph('稳定性：', style='Normal')
    sigma2 = n1 / (psi[ceil(lambda1 / xik1)] * mj)
    if sigma2 < fp:
        mathtemp = r'\frac{N}{\psi A} = \frac{' + rf'{n1}'+'}{'+rf'{psi[ceil(lambda1 / xik1)]} \times {round(mj, 1)}' + '}=' + str(
            round(sigma2, 1)) + r'(MPa) < f =' + f'{fp}(MPa)'
        width = add_image(mathtemp, 'sigma2')
        calc_book.add_paragraph('', style='No Spacing').add_run('').add_picture(f'{path}/sigma2.jpg',
                                                                                width=Inches(width))
